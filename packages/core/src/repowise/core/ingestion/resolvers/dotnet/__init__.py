"""MSBuild / .sln aware resolution helpers for C# / .NET / VB.NET.

The ``DotNetProjectIndex`` is the entry point — it walks a repo once,
parses every ``.csproj``/``.vbproj`` and ``.sln``, builds a namespace → file
map, collects implicit/global/project-level usings, and exposes lookup
helpers used by ``resolvers/csharp.py`` and ``resolvers/vbnet.py``.
...
from .msbuild import MSBuildProject, parse_csproj, parse_project
